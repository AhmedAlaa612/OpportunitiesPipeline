...
• type.subtype: array from [masters, bachelor, phd, internship, fellowship,
  ...
  category rules:
  - "academic": the program's PRIMARY purpose is to fund or enroll a student in a formal degree
    (bachelor / masters / PhD), OR it is a structured research fellowship at a university or
    recognized research institution (e.g. TWAS, EMBO, postdoc programmes).
    KEY: a SCHOLARSHIP that pays for a degree at another university IS academic — it does not
    need to award the degree itself. If the student ends up with a degree, it is academic.
  - "non_academic": the primary outcome is work experience, professional exposure, networking,
    civic engagement, travel, or a short non-degree placement — regardless of the word "scholarship"
    in the title. Includes UN/NGO/corporate work fellowships, internships, conferences, camps,
    leadership programmes (e.g. One Young World), volunteer programmes, workshops, and prizes.
  fellowship disambiguation — the word "fellowship" alone does NOT determine category:
  - fellowship that leads to / funds a degree, or is at a university/research institution → academic
  - fellowship that is essentially a paid professional placement (UN, NGO, embassy, company) → non_academic
  three extra signals to resolve ambiguous cases:
  - AUDIENCE: if the program explicitly targets professionals, practitioners, government
    officials, or mid-career non-students (not degree-seeking students), it is non_academic
    even if the host is a university or well-known scientific body.
  - OUTCOME: if the primary deliverable is a cash prize, recognition award, or certificate
    (not degree enrollment or structured research), it is non_academic.
  - DURATION: programs shorter than 4 weeks are almost always non_academic.
...
<<<<<<< HEAD
    ...
    subtypes = set(type_info.get("subtype") or [])

    if category not in VALID_CATEGORIES:
        issues.append(f"invalid category '{category}' — must be academic or non_academic")

    invalid_subtypes = subtypes - VALID_SUBTYPES
    if invalid_subtypes:
        issues.append(f"invalid subtype value(s): {invalid_subtypes} — not in allowed list")

    if category == "non_academic" and subtypes & DEGREE_SUBTYPES:
        issues.append(
            f"category=non_academic but subtype contains degree level {subtypes & DEGREE_SUBTYPES} "
            "— re-check: a scholarship that funds a degree is academic"
        )

    if category == "academic" and "internship" in subtypes and not (subtypes & DEGREE_SUBTYPES):
        issues.append(
